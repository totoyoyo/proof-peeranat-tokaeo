...
Then if we assume that the theorem is true, we can write $$2=\left(\frac{a}{b}\right)^2.$$
Because $a,b$ are positive, 
	$$\sqrt{2}=\frac{a}{b}.$$
However, this is impossible because $\dfrac{a}{b}$ is a rational number, but $\sqrt{2}$ is not.
So the theorem is disproved with a counterexample.

Secondly, the proof is incorrect because it only is able to prove .... 
If we were to draw a diagram which illustrates the induction of the proof, it would look like this:
