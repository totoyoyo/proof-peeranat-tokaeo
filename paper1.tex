...
However, this is not sufficient because you have to show that there are no possible $a,b,$ or $c >0$ that satisfies the property. 
...
\begin{proof}
	...
	\end{align*}
	We know from the definition of $a_n, b_n$ in Lemma 4 that they are both integers, because the terms in sequences $(a_n)_{n\in\NN}$, $(b_n)_{n\in\NN}$ are constructed simply by adding up previous integer terms, which are initialized at $1$, an integer. 
	Since by assumption $c$ and $d$ are also integers, $\dfrac{c-a_nd}{b_nd}$ is just a fraction of two integers, which is rational. So the RHS is rational, but we know that $\sqrt{2}$, which is the LHS, is irrational. 
